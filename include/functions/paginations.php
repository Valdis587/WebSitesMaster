...
function wp_main_pagination( $args = array() ) {
    ...
    $defaults = array(
        ...
        'before_output'   => '<div class="archive__paginations"><ul class="archive__paginations-list">',
        'after_output'    => '</ul></div>'
    );
    ...
    if ( !empty($min) && !empty($max) ) {
        for( $i = $min; $i <= $max; $i++ ) {
            if ($page == $i) {
                $echo .= '<li><a class="archive__paginations-list-active">' . str_pad( (int)$i, 2, '0', STR_PAD_LEFT ) . '</a></li>';
            } else {
